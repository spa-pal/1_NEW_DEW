...
temp_SS=970-cntrl_stat1;
gran(&temp_SS,30,1010);
LPC_PWM1->MR2=temp_SS;

temp_SS=970-cntrl_stat2;
gran(&temp_SS,30,1010);
...
temp_SL=(signed long)unet_buff_;
temp_SL*=Kunet;
temp_SL/=8000L;
...
if(GET_REG(LPC_ADC->ADGDR,31,1))//ADDR_bit.DONE)
	...
	if(!(adc_cnt&0x01))
		{
		adc_buff[adc_ch][adc_cnt1]=GET_REG(LPC_ADC->ADGDR,4,12);//ADDR_bit.VVDDA;
		if((adc_cnt1&0x03)==0)
			{
			temp_S=0;
			for(i=0;i<16;i++)
               ...
               if(adc_ch!=4)adc_buff_[adc_ch]=temp_S>>4;
               else adc_buff_[adc_ch]=temp_S>>4;
			}
		}
	else 
		{
		unet_buff[adc_cnt1]=GET_REG(LPC_ADC->ADGDR,4,12);//ADDR_bit.VVDDA;
		if((adc_cnt1&0x03)==0)
			{
			temp_S=0;
			for(i=0;i<16;i++)
...
//SET_REG(LPC_GPIO0->FIOCLR,7,5,3);
SET_REG(LPC_GPIO0->FIOPIN,adc_ch,5,3);

//SET_REG(PCONP,1,12,1);
SET_REG(LPC_ADC->ADCR,1,21,1);
SET_REG(LPC_ADC->ADCR,0,16,1);
SET_REG(LPC_ADC->ADCR,24,8,8);

if(adc_cnt&0x01)SET_REG(LPC_ADC->ADCR,1<<4,0,8);//ADCR_bit.SEL=2;
else if(adc_ch<=7)SET_REG(LPC_ADC->ADCR,1<<5,0,8);//ADCR_bit.SEL=8;
else SET_REG(LPC_ADC->ADCR,1<<2,0,8);//ADCR_bit.SEL=8;
