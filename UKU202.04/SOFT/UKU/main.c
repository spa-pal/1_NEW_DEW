...
{
/*if(AUSW_MAIN==6024)		str_of_device_name=	"   ИБЭП220/60-24A   ";
else if(AUSW_MAIN==6012)	str_of_device_name=	"   ИБЭП220/60-12A   ";
else if(AUSW_MAIN==4824)	str_of_device_name=	"   ИБЭП220/48-24A   ";
else if(AUSW_MAIN==4812)	str_of_device_name=	"   ИБЭП220/48-12A   ";
else if(AUSW_MAIN==6010)	str_of_device_name=	"   ИБЭП220/60-10A   ";
else if(AUSW_MAIN==6005)	str_of_device_name=	"   ИБЭП220/60-5A    ";
else if(AUSW_MAIN==4810)	str_of_device_name=	"   ИБЭП220/48-10A   ";
else if(AUSW_MAIN==4805)	str_of_device_name=	"   ИБЭП220/48-5A    ";
else if(AUSW_MAIN==2424)	str_of_device_name=	"   ИБЭП220/24-24A   ";
else if(AUSW_MAIN==2412)	str_of_device_name=	"   ИБЭП220/24-12A   ";
else if(AUSW_MAIN==4840)	str_of_device_name=	"   ИБЭП220/48-40A   ";
else if(AUSW_MAIN==6030)	str_of_device_name=	"   ИБЭП220/60-30A   ";
else if(AUSW_MAIN==4820)	str_of_device_name=	"   ИБЭП220/48-20A   ";
else if(AUSW_MAIN==6015)	str_of_device_name=	"   ИБЭП220/60-15A   ";
else if(AUSW_MAIN==2450)	str_of_device_name=	"   ИБЭП220/24-50A   ";
else if(AUSW_MAIN==2425)	str_of_device_name=	"   ИБЭП220/24-25A   ";	
else if(AUSW_MAIN==2424)	str_of_device_name=	"   ИБЭП220/24-24A   ";
else if(AUSW_MAIN==2412)	str_of_device_name=	"   ИБЭП220/24-12A   ";*/
...
if((sk_stat[0]==skON)&&(!SK_LCD_EN[0]))
	{
	sub_ptrs[i++]=" Сработал шлейф N1  ";
	sub_cnt_max++;	
	}

if((sk_stat[1]==skON)&&(!SK_LCD_EN[1]))
	{
	sub_ptrs[i++]=" Сработал шлейф N2  ";
	sub_cnt_max++;	
	}
	
	/*b_ptrs[i]="   Надпись 1        ";
	sub_cnt_max++;	
	sub_ptrs[i]="     Надпись 2      ";
	sub_cnt_max++;	*/
...
*/
	...
	/*ptrs[9]=		" LAN                ";
     ptrs[10]=		" Спецфункции        ";
     ptrs[11]=		" Журнал событий     ";
	ptrs[12]=		" Батарейный журнал  ";
     ptrs[13]=		" Паспорт            ";
     ptrs[14]=		" Установки          ";
     ptrs[15]=		" Сброс аварий       ";
     ptrs[16]=		sm_exit;*/
...
//	long2lcdhyx(dhcp_tout,2,6,0);
	...
	if((sub_ind1==1)&&(NUMIST==1))
		...
		else                          ptrs[0]=" заблокирован извне ";
		ptrs[1]=" Uист =    @В       "; 
		ptrs[2]=" Iист =    #А       ";
...
//	else if((AUSW_PDH<0)||(AUSW_PDH>=1000))sub_bgnd("не установлен",'%',-5);
	...
	{
	/*
const char sm301[]	={" Стандартные        "};
const char sm302[]	={" Основной источн. N<"};
const char sm303[]	={" Umax=       !В     "}; 
const char sm304[]	={" Uб0=        @В     "};
const char sm305[]	={" Uб20=       #В     "};	
const char sm306[]	={" tmax=       $°C    "}; 
const char sm307[]	={" Cбат=       %а*ч   "}; 
const char sm308[]	={" Uсигн=      ^В     "};
const char sm309[]	={" Umin.сети=  &В     "}; 
const char sm310[]	={" Зв.сигн.   (       "}; 
const char sm311[]	={" Iбк.=       )А     "};
const char sm312[]	={" Kвыр.зар.=    [    "}; 
const char sm313[]	={" Imax=       ]A     "};
const char sm314[]	={" Kimax=      {      "};
const char sm315[]	={" АПВ источников     "};
const char sm316[]	={" Iз.мах.=    <А     "};
const char sm317[]	={" U0б=        >В     "};
const char sm318[]	={" Tз.вкл.а.с. !с     "};
...
*/
	...
	ptrs[3]=" Сухой контакт N1   ";
	ptrs[4]=" Сухой контакт N2   ";
	ptrs[5]=" Сухой контакт N3   ";
	ptrs[6]=" Сухой контакт N4   ";
	ptrs[7]=sm_exit;
...
av_rele=0x0000;
 ...
 if(but==butUD)	 
	...
	if(but==butD)
		...
		{
		snmp_trap_send("Мама мыла раму",0xffff);
		}
	else if(but==butD_)
		{
		     tree_up(iLog,0,0,0);
...
*/		
	...
	if(but==butD)
		...
		if(spc_stat==spc_VZ)
			...
			spc_stat=spc_VZ;
			cnt_vz_sec=0L;
			cnt_vz_sec_=(/*SEC_IN_HOUR*/4000*VZ_HR);
			vz_mem_hndl(1);
			}
		} 	  		
	else if(sub_ind==2)
